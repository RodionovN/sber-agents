...
async def get_transaction_response_voice(
    ...
    """Обработка транскрибированного текста из голосового сообщения со structured output."""
    try:
        logger.info(f"Using model for voice: {config.MODEL_TEXT}, base_url: {config.OPENAI_BASE_URL}")
        response = await client.chat.completions.create(
            model=config.MODEL_TEXT,
            messages=[
                {"role": "system", "content": config.SYSTEM_PROMPT_TEXT},
        ...
        logger.error(f"Model not found for voice: {config.MODEL_TEXT}, base_url: {config.OPENAI_BASE_URL}")
        logger.error(f"LLM API error: {e}")
        raise ValueError(f"Модель '{config.MODEL_TEXT}' не найдена. Проверьте настройки MODEL_TEXT в .env файле.")
    except (APIError, InternalServerError) as e:
