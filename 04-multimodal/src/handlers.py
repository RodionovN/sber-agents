...
async def handle_voice(message: Message):
    ...
    try:
        ...
        temp_file_path = temp_dir / f"{voice.file_id}.ogg"
        ...
        transcribed_text = await transcribe_voice(str(temp_file_path))
        logger.info(f"Transcribed text from {chat_id}: {transcribed_text[:100] if transcribed_text else 'EMPTY'}...")
        ...
        if not transcribed_text or not transcribed_text.strip():
            await message.answer(
                ...
                "• Убедиться, что в сообщении есть речь\n"
                "• Попробовать еще раз"
            )
            return
        ...
        # Получаем ответ LLM с structured output
        logger.info(f"Calling LLM for voice with model: {config.MODEL_TEXT}, base_url: {config.OPENAI_BASE_URL}")
        ...
        logger.error(f"Configuration error for voice from {chat_id}: {e}", exc_info=True)
        error_msg = str(e)
        if "модель" in error_msg.lower() or "model" in error_msg.lower():
            await message.answer(
                f"❌ Ошибка конфигурации модели: {error_msg}\n\n"
                "Проверьте настройки MODEL_TEXT в .env файле.\n\n"
                "Для Ollama на сервере используйте:\n"
                "• MODEL_TEXT=qwen2.5:7b-instruct\n"
                "• или MODEL_TEXT=llama3.2:1b\n\n"
                "И убедитесь, что:\n"
                "• OPENAI_BASE_URL=http://195.209.214.199:11434/v1"
            )
        else:
            await message.answer(
                f"Ошибка конфигурации транскрибации: {error_msg}\n\n"
                "Проверьте настройки TRANSCRIPTION_PROVIDER и соответствующий API ключ в .env файле."
            )
    except NotFoundError as e:
        logger.error(f"Model not found error for voice from {chat_id}: {e}", exc_info=True)
        await message.answer(
            "❌ Модель не найдена.\n\n"
            "Проверьте настройки MODEL_TEXT в .env файле.\n\n"
            "Для Ollama на сервере используйте:\n"
        ...
        await message.answer(
            "❌ Ошибка при обработке файла голосового сообщения.\n\n"
            "Попробуйте отправить голосовое сообщение еще раз."
        )
    except Exception as e:
        ...
        if "подключения" in error_str.lower() or "connection" in error_str.lower():
            await message.answer(
                "❌ Ошибка подключения к серверу транскрибации.\n\n"
                "Проверьте, что сервер транскрибации доступен:\n"
                "• WHISPER_SERVER_URL=http://195.209.214.199:8080"
            )
        elif "пустой текст" in error_str.lower() or "empty" in error_str.lower():
            await message.answer(
                "❌ Не удалось распознать речь в голосовом сообщении.\n\n"
                "Попробуйте:\n"
                "• Говорить четче и громче\n"
            ...
            await message.answer(
                f"❌ Произошла ошибка при обработке голосового сообщения: {error_str}\n\n"
                "Попробуйте еще раз или используйте /start для начала нового диалога."
            )
    finally:
