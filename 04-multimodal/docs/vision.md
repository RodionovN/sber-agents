...
│   ├── transcription.py # Транскрибация голосовых сообщений
...
2. **handlers.py** - обработка событий
   ...
   - Обработчик голосовых сообщений → транскрибация через Speech-to-Text API → извлечение транзакций через LLM → сохранение транзакций → показ ответа + статус + баланс
...
3. **llm.py** - интеграция с LLM
   ...
   - Метод `get_transaction_response_voice()` - обработка транскрибированного текста из голосовых сообщений со structured output
...
4. **transcription.py** - транскрибация голосовых сообщений
   - Метод `transcribe_voice()` - преобразование голосового сообщения в текст через Speech-to-Text API
   - Поддержка различных провайдеров (Yandex SpeechKit, OpenAI Whisper API, локальный Whisper)
...
6. **config.py** - конфигурация
   - Класс Config с полями: `TELEGRAM_TOKEN`, `OPENAI_API_KEY`, `OPENAI_BASE_URL`, `MODEL_TEXT`, `MODEL_IMAGE`, `SYSTEM_PROMPT_TEXT`, `SYSTEM_PROMPT_IMAGE`, `TRANSCRIPTION_PROVIDER`, `YANDEX_SPEECHKIT_API_KEY`, `OPENAI_WHISPER_API_KEY`
   ...
   - `TRANSCRIPTION_PROVIDER` - провайдер транскрибации (yandex, openai, local_whisper)
...
```
Telegram → handlers.py (голосовое сообщение → скачать файл) → transcription.py (Speech-to-Text API) → 
transcription.py → handlers.py (транскрибированный текст) → llm.py (structured output) → OpenRouter/Ollama → 
llm.py → handlers.py (извлечь транзакции, сохранить в transactions, показать ответ + статус + баланс) → Telegram
```
...
2. Бот скачивает голосовой файл из Telegram
3. Бот отправляет файл в Speech-to-Text API (Yandex SpeechKit или другой провайдер)
4. Получает транскрибированный текст
5. Бот отправляет транскрибированный текст в LLM со structured output
6. LLM извлекает транзакции и возвращает TransactionResponse
...
# Настройки транскрибации голосовых сообщений
TRANSCRIPTION_PROVIDER=yandex  # yandex, openai, local_whisper
YANDEX_SPEECHKIT_API_KEY=
OPENAI_WHISPER_API_KEY=  # если используете OpenAI Whisper API
```
