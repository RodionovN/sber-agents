...
Модель автоматически скачивается при первом запуске, но если возникают проблемы с загрузкой, можно скачать её заранее:

```bash
# Способ 1: Использовать скрипт проекта (рекомендуется)
make download-embedding-model

# Или напрямую:
uv run python scripts/download_embedding_model.py

# Способ 2: Использовать huggingface-cli (если установлен)
huggingface-cli download intfloat/multilingual-e5-base --local-dir ./models/multilingual-e5-base

# Способ 3: Использовать локальную модель после скачивания
# В .env укажите путь к локальной папке:
HUGGINGFACE_EMBEDDING_MODEL=./models/multilingual-e5-base
```

**Примечание:** Если возникают проблемы с таймаутами при скачивании, см. подробную инструкцию: [docs/download_model_instructions.md](docs/download_model_instructions.md)

Кратко:
- Использовать VPN или другое интернет-соединение
- Скачать модель вручную с [HuggingFace Hub](https://huggingface.co/intfloat/multilingual-e5-base)
- Использовать альтернативную модель меньшего размера
