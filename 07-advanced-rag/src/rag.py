...
def get_cross_encoder():
    ...
    if cross_encoder is None:
        ...
        # Если модель не указана, используем значение по умолчанию
        if not model_path:
            model_path = "cross-encoder/mmarco-mMiniLMv2-L12-H384-v1"
            logger.info(f"Cross-encoder model not specified, using default: {model_path}")
        
        # Проверяем, является ли путь локальным
        model_path_obj = Path(model_path) if model_path else None
        is_local_path = False
        
        if model_path_obj and model_path_obj.exists() and model_path_obj.is_dir():
            ...
            if config_file.exists() and (model_file.exists() or pytorch_file.exists()):
                is_local_path = True
                model_path = str(model_path_obj.resolve())
                logger.info(f"Using local cross-encoder model: {model_path}")
            else:
                logger.warning(f"Local path {model_path} exists but model files not found, using HuggingFace Hub")
                model_path = "cross-encoder/mmarco-mMiniLMv2-L12-H384-v1"
        ...
        try:
            ...
            error_msg = str(e)
            # Если локальная модель повреждена или несовместима, используем HuggingFace Hub
            if is_local_path or ("no file named" in error_msg or "size mismatch" in error_msg or 
                                 "Can't load" in error_msg or "not a local folder" in error_msg):
                logger.warning(
                    f"⚠️  Локальная модель {model_path} не может быть загружена: {error_msg[:200]}"
                )
                logger.info(f"Переключаемся на модель из HuggingFace Hub: cross-encoder/mmarco-mMiniLMv2-L12-H384-v1")
                try:
                    hub_model_path = "cross-encoder/mmarco-mMiniLMv2-L12-H384-v1"
                    cross_encoder = CrossEncoder(hub_model_path)
                    logger.info("✓ Cross-encoder loaded successfully from HuggingFace Hub")
                except Exception as e2:
                    logger.error(f"Failed to load cross-encoder from HuggingFace Hub: {e2}", exc_info=True)
                    raise
            else:
