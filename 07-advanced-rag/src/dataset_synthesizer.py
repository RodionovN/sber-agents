...
def synthesize_qa_pairs_from_pdf(chunks: List, llm_model: str = None) -> List[Dict[str, Any]]:
    ...
    if llm_model is None:
        llm_model = config.MODEL or "gpt-4o"
    
    # Создаем LLM с настройками из конфига
    llm_kwargs = {
        "model": llm_model,
        "temperature": 0.7,
        "max_tokens": 4000  # Ограничение для бесплатного аккаунта OpenRouter (лимит 8000 токенов)
    }
...
def create_dataset(data_dir: str, samples_per_file: int = 2) -> List[Dict[str, Any]]:
    ...
    # Используем модель из конфига для синтеза
