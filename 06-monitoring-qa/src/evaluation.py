...
def init_ragas_metrics():
    ...
    if config.RAGAS_EMBEDDING_PROVIDER == "ollama":
        ...
        logger.info(f"Using Ollama embeddings for RAGAS: {config.RAGAS_EMBEDDING_MODEL} (base_url: {config.OLLAMA_BASE_URL})")
    elif config.RAGAS_EMBEDDING_PROVIDER == "huggingface":
        from langchain_huggingface import HuggingFaceEmbeddings
        from pathlib import Path
        
        # Проверяем, является ли путь локальным
        model_path = Path(config.RAGAS_EMBEDDING_MODEL)
        if model_path.exists() and model_path.is_dir():
            ...
            if has_model_files:
                ...
                logger.info(f"Using local HuggingFace model for RAGAS: {model_name}")
            else:
                ...
                logger.warning(
                    ...
                    f"This might be an Ollama model. Consider using RAGAS_EMBEDDING_PROVIDER=ollama instead."
                )
                raise ValueError(
                    f"No HuggingFace model files found in {model_path}. "
                    f"If this is an Ollama model, set RAGAS_EMBEDDING_PROVIDER=ollama"
                )
        else:
            # Используем имя модели из HuggingFace Hub
            model_name = config.RAGAS_EMBEDDING_MODEL
            logger.info(f"Using HuggingFace Hub model for RAGAS: {model_name}")
        ...
        logger.info(f"Using HuggingFace embeddings for RAGAS: {model_name}")
    else:
