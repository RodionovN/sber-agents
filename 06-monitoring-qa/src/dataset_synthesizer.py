...
def synthesize_qa_pairs_from_pdf(chunks: List, llm_model: str = None) -> List[Dict[str, Any]]:
    ...
    Args:
        ...
        llm_model: модель для синтеза (если None, используется config.MODEL)
    ...
    if not llm_model:
        logger.error(
            "MODEL is not configured. Please set MODEL in your .env file.\n"
            "For Fireworks.ai example: MODEL=accounts/fireworks/models/gpt-oss-120b\n"
            "For OpenRouter example: MODEL=openai/gpt-oss-20b:free"
        )
        raise ValueError("MODEL configuration is required")
    
    # Инициализируем LLM с учетом base_url из конфига (для Fireworks, OpenRouter и т.д.)
    llm_kwargs = {
        "model": llm_model,
        "temperature": 0.7
    }
    
    # Если указан base_url в конфиге, используем его
    if config.OPENAI_BASE_URL:
        llm_kwargs["base_url"] = config.OPENAI_BASE_URL
    
    llm = ChatOpenAI(**llm_kwargs)
    logger.info(f"Using LLM model: {llm_model} (base_url: {config.OPENAI_BASE_URL or 'default'})")
  ...
  "qa_pairs": [
    ...
    for i, chunk in enumerate(chunks):
        ...
        try:
            ...
            error_msg = str(e)
            # Проверяем, является ли это ошибкой модели (404)
            if "404" in error_msg or "NOT_FOUND" in error_msg or "Model not found" in error_msg:
                logger.error(
                    f"Model not found error for chunk {i}. "
                    f"Model: {llm_model}, Base URL: {config.OPENAI_BASE_URL or 'default'}. "
                    f"Please check your MODEL configuration in .env file."
                )
                # Если это первая ошибка такого типа, выводим подсказку
                if i == 0:
                    logger.error(
                        "\n" + "="*60 + "\n"
                        "ERROR: The specified model is not available.\n"
                        f"Current MODEL setting: {config.MODEL}\n"
                        f"Current BASE_URL setting: {config.OPENAI_BASE_URL or 'default'}\n"
                        "Please check:\n"
                        "1. Model name is correct for your API provider\n"
                        "2. Model is deployed/available in your account\n"
                        "3. API key has access to this model\n"
                        "="*60 + "\n"
                    )
            else:
...
def create_dataset(data_dir: str, samples_per_file: int = 2) -> List[Dict[str, Any]]:
    ...
    # Используем модель из конфига (или можно передать явно)
    pdf_qa_pairs = synthesize_qa_pairs_from_pdf(pdf_chunks, llm_model=config.MODEL)
